...
class App extends Component {
  ...
  async handleContribution(event)
 {
   if (typeof this.state.bountiesInstance !== 'undefined') {
     ...
     await this.state.bountiesInstance.methods.contributorDeposit().send({from: this.state.account, value: this.state.web3.utils.toWei(this.state.contributeAmount, 'ether')})
   }
 }
 ...
 async handleClaim(event)
{
  if (typeof this.state.bountiesInstance !== 'undefined') {
    ...
    await this.state.bountiesInstance.methods.contributorClaim().send({from: this.state.account})
  }
}
  ...
  render() {
    ...
    return (
      ...
      <Grid>
      <Row>
      <a href={"https://ropsten.etherscan.io/address/"+this.state.bountiesInstance.address} target="_blank" rel="noopener noreferrer">Contract Address</a>
      </Row>
      <br/>
      <Row>
      <Panel>
